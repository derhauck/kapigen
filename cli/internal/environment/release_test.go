...
func TestIsRelease(t *testing.T) {
	...
	t.Run("will work with ci vars", func(t *testing.T) {
		...
		CI_COMMIT_TAG.Set("1.0.0")
		if !IsRelease() {
			t.Errorf("should be true as env was prepared id: %s, commit: %s, default :%s, tag:%s", CI_MERGE_REQUEST_ID.Get(), CI_COMMIT_BRANCH.Get(), CI_DEFAULT_BRANCH.Get(), CI_COMMIT_TAG.Get())
		}
	})
	t.Run("will not work without tag", func(t *testing.T) {
		CI_MERGE_REQUEST_ID.Set("123")
		CI_COMMIT_BRANCH.Set("feature")
		CI_DEFAULT_BRANCH.Set("master")
		CI_COMMIT_TAG.Unset()
		if IsRelease() {
			t.Errorf("should be false as tag is not set, %s", CI_COMMIT_TAG.Get())
		}
	})
}
